...
RegisterNetEvent("mb-GetRichPlayer:server:getTopPlayerMoney", function(type)
    if (type == "full") then
        local topRichestPlayers = MySQL.Sync.fetchAll("SELECT `name`, `money`, `citizenid`, `license`, JSON_VALUE(money, '$.cash') + JSON_VALUE(money, '$.bank') AS `total_money` FROM `players` GROUP BY `citizenid` ORDER BY `total_money` DESC LIMIT ?", {Config.OnlyTopRichest.top})
    ...
    elseif (type == "standard") then
        local topRichestPlayers = MySQL.Sync.fetchAll("SELECT `name`, `money`, `citizenid`, JSON_VALUE(money, '$.cash') + JSON_VALUE(money, '$.bank') AS `total_money` FROM `players` GROUP BY `citizenid` ORDER BY `total_money` DESC LIMIT ?", {Config.OnlyTopRichest.top})
